...
## 总结
基本流程如下：  
1. 通过Observer对象深度遍历data，为所有属性通过`Object.definePropert`生成`setter/getter`,并为每个属性创建一个用于`Dep`实例用于维护引用了当前属性的`Watch`列表。在getter中收集依赖。setter中触发`dep.notify`派发更新操作，执行_render()。
